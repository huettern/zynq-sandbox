...
additional_firmware_tar="linux-firmware-ath9k_htc-20200519-r0.apk linux-firmware-brcm-20200519-r0.apk linux-firmware-rtlwifi-20200519-r0.apk"
...
echo "# Untar"
# Additional drivers
for tar in $additional_firmware_tar
do
...
echo "This will ask for sudo to chroot to alpine root dir"
...
zipfile=red-pitaya-alpine-3.9-armv7-`date +%Y%m%d`.zip
zip -r $zipfile $zip_dir/

echo "######################################################################"
echo "# DONE!"
echo "Extract contents of $zipfile to a sd card BOOT FAT partition"